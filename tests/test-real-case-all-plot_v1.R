...
setwd("C:/Users/Ze/OneDrive - UNSW/R_Package/WASP/tests/realcaseall")
#setwd("H:/10_Drought_Prediction/Wavelets_EMS_Paper/WASP/tests/realcaseall")
  ...
  for(i in 1){
    for(j in c("origin","vt")){
      ...
      p <- spplot(SPI.obs.ras, xlim=c(110,156), ylim=c(-45,-9),
                  ...
                  sp.layout = list("sp.polygons",Aus_map, first=FALSE, col="grey", fill="transparent"),
...
fig3 <- plot_grid(plotlist = p3.list, nrow=2, labels=c("(a)","(b)","(c)","(d)"),
                  ...
                  scale=c(1.2,0.8,1.2,0.8), rel_widths = c(1.2,0.8,1.2,0.8))
fig3
