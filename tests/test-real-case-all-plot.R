...
#setwd("H:/10_Drought_Prediction/Wavelets_EMS_Paper/WASP/tests/realcase1")
...
p3.list <- list() #Density plot
p4.list <- list() #Taylor Diagram
for(sc in sc.list){
#response - obs
...
for(i in 1:length(Grids)){
  ...
  for(j in 1:length(sel.cv)){
    tmp1 <- c(tmp1, as.numeric(as.character(sel.cv[[j]]$origin[[i]])))
    tmp2 <- c(tmp2, as.numeric(as.character(sel.cv[[j]]$vt[[i]])))
  }
...
#----------------------------------------------
###barplot
df1 <- cbind(Group=1,Grids, SPI.ref.PDF[,Grids])
df2 <- cbind(Group=2,Grids, SPI.mod.PDF[,Grids])
...
p2<-ggplot(df.PDF,aes(x=factor(No),y=PDF)) +
  ...
  scale_x_discrete(labels=paste0("Sampled Grid: ",sort(Grids))) +
...
fig1 <- plot_grid(p1.list[[1]], p1.list[[2]], ncol=2, labels=labels,label_y=0.9, label_x=0.05,
                           scale = c(1,1),label_size=10, rel_widths = c(1, 1))
fig1
