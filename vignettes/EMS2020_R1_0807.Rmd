...
```{r cod, eval=FALSE, message=TRUE, warning=TRUE}
...
dwt <- modwt.vt(data, wf=wf, J=8, boundary="periodic", cov.opt = "auto")
    ...
    for(i in 1){
      for(j in c("origin","vt")){
        ...
        p <- ggplot(data=na.omit(Sel.df), aes(x=lon, y=lat)) + 
             ...
             geom_polygon(data=aus.coast, aes(x = long, y = lat, group=group), color="grey", fill="NA")+
...
for(sc in c(12,36)){
    ...
    for(data in c("SPI.mod.PDF.OL")){
      ...
      sl2 <- list("sp.polygons",aus.coast, first=FALSE, col="grey")
      ...
      #df.ras.mask <- mask(SPI.obs.ras, aus.coast)
...
if(TRUE){
  ...
  par(mfrow=c(ndim+1,1),mar=c(2,4,2,2),bg = "white",pty="m",ps=8)
...
#grids.ras
#---------------------------------------
sl1 <- list("sp.polygons",aus.coast, first=FALSE, col="grey")
